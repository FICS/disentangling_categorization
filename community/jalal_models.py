...
# hard_label_manifiolds/community/jalal_simple_models.py 
...
def jalal_bn(pretrained=False):
    features = jalal_bn_features(pretrained=False)
    model = MNISTClassifier(features, not pretrained)
    if pretrained:
        my_dict = torch.load(model_file)
...
def jalal(pretrained=False):
    features = jalal_features(pretrained=False)
    model = MNISTClassifier(features, not pretrained)
    if pretrained:
        my_dict = torch.load(model_file)
        model.load_state_dict(my_dict)
