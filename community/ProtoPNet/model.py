...
from .jalal_features import jalal_features, jalal_bn_features
...
base_architecture_to_features = {'resnet18': resnet18_features,
                                 ...
                                 'jalal': jalal_features,
                                 'jalal_bn': jalal_bn_features,}
...
class PPNet(nn.Module):
    ...
    def __init__(self, features, img_size, prototype_shape,
        ...
        if features_name.startswith('VGG') or features_name.startswith('RES') or features_name.startswith('MNIST'):
...
def construct_PPNet(base_architecture, pretrained=True, img_size=224,
    ...
    if pretrain_ckpt is None:
        ...
        # ProtoPNetB path
        features = base_architecture_to_features[base_architecture](pretrained=False)
        my_dict = torch.load(pretrain_ckpt)['state_dict']
        if 'resnet' in base_architecture:
            my_dict.pop('fc.weight')
            my_dict.pop('fc.bias')
        elif 'vgg' in base_architecture:
            keys_to_remove = set()
            for key in my_dict:
            ...
            for key in keys_to_remove:
                del my_dict[key]
        elif 'densenet' in base_architecture:
            pattern = re.compile(
            ...
            del my_dict['classifier.weight']
            del my_dict['classifier.bias']

        features.load_state_dict(my_dict)
