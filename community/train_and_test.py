...
def _train_or_test(model, dataloader, optimizer=None, class_specific=True, use_l1_mask=True,
    ...
    xent = total_cross_entropy / n_batches
    ...
    if class_specific:
        ...
        log('\tseparation:\t{0}'.format(separation_cost))
        log('\tavg separation:\t{0}'.format(avg_separation_cost))
    ...
    log('\tcross ent: \t{0}'.format(xent))
    log('\tcluster: \t{0}'.format(cluster_cost))
    log('\taccu: \t\t{0}%'.format(accu))
    log('\tl1: \t\t{0}'.format(l1))
    log('\tp dist pair: \t{0}'.format(p_avg_pair_dist))
    
    return xent, cluster_cost, accu, l1, p_avg_pair_dist, separation_cost, avg_separation_cost
